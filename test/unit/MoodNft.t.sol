// SPDX-License-Identifier: MIT
pragma solidity ^0.8.20;

import { Test, console2 } from "forge-std/Test.sol";
import { MoodNft } from "../../src/MoodNft.sol";

contract MoodNftTest is Test {
    MoodNft public moodNft;
    address public USER = makeAddr("user");

    string public constant HAPPY_SVG_URI =
        "data:image/svg+xml;base64,";
    string public constant SAD_SVG_URI =
        "data:image/svg+xml;base64,";

    function setUp() public {
        // deployer = new DeployMoodNft(HAPPY_SVG_URI, SAD_SVG_URI);
        // moodNft = deployer.run();
        moodNft = new MoodNft(HAPPY_SVG_URI, SAD_SVG_URI);
        vm.deal(USER, 1 ether);
    }

    function testMoodNftIsCorrectlyDeployed() public view {
        assertEq(moodNft.name(), "Mood NFT");
    }

    function testViewTokenURI() public {
        vm.prank(USER);
        moodNft.mintNft();

        string memory actualUri = moodNft.tokenURI(0);
        console2.log("actualUri", actualUri);

        // 验证返回的是 JSON 格式的 base64 URI
        assertTrue(bytes(actualUri).length > 0, "URI should not be empty");

        // 验证前缀是正确的
        string memory expectedPrefix = "data:application/json;base64,";
        bytes memory actualBytes = bytes(actualUri);
        bytes memory prefixBytes = bytes(expectedPrefix);

        assertTrue(actualBytes.length >= prefixBytes.length, "URI too short");

        for (uint256 i = 0; i < prefixBytes.length; i++) {
            assertEq(actualBytes[i], prefixBytes[i], "Prefix mismatch");
        }
    }

    function testTokenURIContainsCorrectImageData() public {
        vm.prank(USER);
        moodNft.mintNft();

        string memory tokenURI = moodNft.tokenURI(0);

        // 解码 base64 获取 JSON
        string memory prefix = "data:application/json;base64,";
        bytes memory tokenURIBytes = bytes(tokenURI);
        bytes memory prefixBytes = bytes(prefix);

        // 提取 base64 部分
        bytes memory base64Part = new bytes(tokenURIBytes.length - prefixBytes.length);
        for (uint256 i = 0; i < base64Part.length; i++) {
            base64Part[i] = tokenURIBytes[i + prefixBytes.length];
        }

        // 验证 base64 部分不为空
        assertTrue(base64Part.length > 0, "Base64 part should not be empty");

        // 验证包含了 SAD SVG URI（因为默认mood是SAD）
        assertTrue(bytes(tokenURI).length > bytes(prefix).length, "Token URI should contain base64 data");
    }
}
